...
// Initialize database tables (assumes database already exists in cPanel)
const initDatabase = async () => {
  try {
    ...
    await new Promise((resolve, reject) => {
      pool.query(createSurveyTable, (err) => {
        if (err) reject(err);
        else resolve();
      });
    });
    ...
    // Create users table (full schema)
    ...
    const insertAdmin = `
      INSERT IGNORE INTO users (username, email, password, role, verified) VALUES 
      ('contactflowair@gmail.com', 'contactflowair@gmail.com', 'ab', 'admin', TRUE)
    `;
    
    await new Promise((resolve, reject) => {
      pool.query(insertAdmin, (err) => {
        if (err) reject(err);
        else resolve();
      });
    });
    ...
    console.error('Make sure the database exists and the user has permissions in cPanel.');
  }
};
